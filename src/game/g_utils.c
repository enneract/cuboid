...
recalc client's STAT_MISC
===============
*/

void G_RecalcBuildTimer( gclient_t *client )
{
  if(BG_InventoryContainsWeapon(WP_ABUILD,client->ps.stats)||
     BG_InventoryContainsWeapon(WP_ABUILD2,client->ps.stats)||
     BG_InventoryContainsWeapon(WP_HBUILD,client->ps.stats))
    client->ps.stats[STAT_MISC]=MIN(client->buildTimer,MAXIMUM_BUILD_TIME);
}
